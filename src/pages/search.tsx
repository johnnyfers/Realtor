...
export default function Search({properties}) {
    ...
    return (
        <Box>
            ...
            <Text fontSize='30' p='4' fontWeight='bold'>
                Properties {router.query.purpose}
            </Text>
            <Flex flexWrap='wrap'>
            {properties.map((property: IProperty) => <Property {...property} key={property.id}/>)}
            </Flex>
            {properties.length === 0 && (
                  <Flex justifyContent='center' alignItems='center' flexDir='column' marginTop='5' marginBottom='5' >
                  <Image src={noresult} alt={'no-result'} />
                    <Text marginTop={'3'} fontSize="20">No Results Found</Text>
                </Flex>
            )}
        </Box>
    )
